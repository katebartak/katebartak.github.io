...
const BUILD_IN_THEME = false; // Set to true when prototype inside cms environment

// General theme globs to the files.
// This globs used for copy prototype files to theme.
...
let scss = () => {
  return gulp
    ...
    .pipe(gulpif(BUILD_IN_THEME, gulp.dest(globs.theme + 'css/')))
    .pipe(server.stream());
};
...
let scripts = () => {
  return gulp
    ...
    .pipe(gulpif(BUILD_IN_THEME, gulp.dest(globs.theme + 'js/')))
    .pipe(server.stream());
};
...
gulp.task('build', function() {
  return gulp
    ...
    .pipe(gulp.dest(globs.theme + 'css/'));
});
