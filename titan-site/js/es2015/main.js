...
(function ($) {
  ...
  (function initHomeObjectsSlider() {
    ...
    } else if ($slider.find('.object-link').length) {
  ...
  (function ChooseCityLinks() {
    ...
    $chooseLink.on('click', function (event) {
      event.preventDefault(); // var siteUrl = window.location.protocol + "//" + window.location.host + window.location.pathname;
  ...
  (function productExpandedText() {
    ...
    $text.attr('data-height', $text.height()).addClass('expand-active').animate({
      'height': 245
    }, 100);
    $expandButton.on('click', function () {
      ...
      $targetText.animate({
        'height': $targetText.data('height')
      }, 500, function () {
        $targetText.removeAttr('style').removeClass('expand-active');
      });
    });
  })();
  ...
  (function firstscreenExpandText() {
    ...
    $expandButton.on('click', function () {
      $(this).parents('.firstscreen-note').removeClass('collapsed-text').addClass('expanded-text');
    });
  })();
  ...
  (function lazyLoadingImages() {
    var MSIE8 = /msie|trident/i.test(navigator.userAgent); // var MSIE8 = (jQuery.browser.msie) && (jQuery.browser.version == 8);
