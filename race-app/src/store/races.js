...
const actions = {
  ...
  async startRace({ commit, state }, raceIndex) {
    ...
    const slowestHorseCondition = Math.min(
      ...race.participants.map((item) => item.condition),
    );
    await new Promise((resolve) =>
      setTimeout(
        resolve,
        raceTimingMixin.methods.calculateAnimationDuration(
          race.distance,
          slowestHorseCondition,
          state.trackWidth,
        ) + 1000,
      ),
    );
    ...
    const results = race.participants
      .map((horse) => {
        const baseTime = race.distance / 300; // Base speed (lower = faster)
        const conditionFactor = (horse.condition / 100) * 0.3; // Better condition = faster
        const randomFactor = Math.random() * 0.2; // Random factor up to 20%
