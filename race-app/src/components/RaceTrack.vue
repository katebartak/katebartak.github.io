<template>
  <CardHolder
    :title="
      currentRace
        ? `${currentRace.distance}m Race - Round ${currentRace.id}`
        : 'Race Track'
    "
  >
...
import { raceTimingMixin } from "@/mixins/race-timing.mixin";
...
export default {
  ...
  mixins: [raceTimingMixin],
  computed: {
    ...mapGetters("races", ["currentRace", "isRacing", "trackWidth"]),
  },
  mounted() {
  ...
  methods: {
    ...
    updateTrackWidth() {
      this.$nextTick(() => {
        const trackSurfaces = this.$refs.trackSurface;
        const firstTrackSurface = Array.isArray(trackSurfaces)
          ? trackSurfaces[0]
          : trackSurfaces;
        const trackWidthValue = firstTrackSurface
          ? firstTrackSurface.offsetWidth
          : 500;

        // Save to store
        this.$store.dispatch("races/setTrackWidth", trackWidthValue);

        // Set CSS custom property
        this.$el.style.setProperty("--track-width", `${trackWidthValue}px`);
      });
    },
    handleResize() {
      this.updateTrackWidth();
...
.horse-icon.racing {
  animation: gallop var(--animation-duration) linear var(--animation-delay)
    forwards;
}
